...
function ngg_ajax_navigation(e, obj) {
  ...
  if (jQuery(currentNode.parentNode).attr('id')) {
    var gallery = jQuery(currentNode.parentNode); // we found a gallery, let's extract the post id & gallery id
    ...
    ngg_show_loading(e); // load gallery content

    jQuery.get(ngg_ajax.callback, {
      p: postId,
      galleryid: galleryId,
      nggpage: pageNumber,
      type: 'gallery'
      ...
      // delete old content
      gallery.children().remove(); // add new content

      gallery.replaceWith(data); // add ajax-navigation, again

      jQuery('document').ready(function () {
        // remove old listeners to avoid double-clicks
        jQuery('a.page-numbers').unbind('click');
        jQuery('a.prev').unbind('click');
        jQuery('a.next').unbind('click'); // add shutter-listeners again
...
function ngg_ajax_browser_navigation(e, obj) {
  ...
  if ('ngg-prev-' == jQuery(obj).attr('id').substr(0, 9) || 'ngg-next-' == jQuery(obj).attr('id').substr(0, 9)) {
    ...
    var imageNumber = jQuery(obj).attr('id').substr(9); // find the image-browser-container
    ...
    if (jQuery(currentNode.parentNode).hasClass('ngg-imagebrowser')) {
      var gallery = jQuery(currentNode.parentNode); // let's extract the post id & gallery id
      ...
      ngg_show_loading(e); // get content
      ...
      jQuery.get(ngg_ajax.callback, {
        ...
        jQuery('document').ready(function () {
          ...
          jQuery('a.ngg-browser-prev').unbind('click'); // add shutter-listeners again

          shutterReloaded.init('sh'); // register ajax browser-navigation listeners
