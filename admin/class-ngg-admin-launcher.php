...
class NGG_Admin_Launcher {
	...
	public function add_menu() {
		add_menu_page(
			__( 'Galleries', 'nggallery' ),
			__( 'Galleries', 'nggallery' ),
			'NextGEN Gallery overview',
			NGGFOLDER,
			array( $this, 'show_menu' ),
			$this->get_icon_gallery()
		);
		...
		add_submenu_page(
			NGGFOLDER,
			__( 'Add Gallery / Images', 'nggallery' ),
			__( 'Add Gallery / Images', 'nggallery' ),
	...
	public function add_network_admin_menu() {
		add_menu_page(
			...
			'nggallery-wpmu',
			NGGFOLDER,
			array( $this, 'show_network_settings' ),
			$this->get_icon_gallery()
		);

		add_submenu_page(
			NGGFOLDER,
			__( 'Network settings', 'nggallery' ),
			__( 'Network settings', 'nggallery' ),
			'nggallery-wpmu',
			NGGFOLDER,
			array( $this, 'show_network_settings' )
	...
	public function load_scripts() {
		...
		wp_register_script(
			'ngg-progressbar',
			plugins_url( 'js/ngg.progressbar.js', __FILE__ ),
			array( 'jquery' ),
			'2.0.1'
		);
		...
		switch ( $_GET['page'] ) {
			...
			case 'nggallery-manage':
				...
				wp_enqueue_script( 'ngg-cropper', NGGALLERY_URLPATH . 'public/js/cropper.min.js', '2.2.5' );
				wp_register_script( 'shutter', NGGALLERY_URLPATH . 'public/js/shutter-reloaded.min.js', false, '1.3.2' );
				wp_localize_script(
					'shutter',
					'shutterSettings',
					array(
	...
	public function load_styles() {
		...
		switch ( $_GET['page'] ) {
			...
			case 'nggallery-options':
				wp_enqueue_style(
					'nggtabs',
					plugins_url( 'css/jquery.ui.tabs.css', __FILE__ ),
					false,
					'2.5.0',
					'screen'
				);
				...
				break;
			case 'nggallery-manage':
				wp_enqueue_style( 'ngg-cropper', NGGALLERY_URLPATH . 'public/css/cropper.min.css', false,'2.2.5' );
				wp_enqueue_style(
					'shutter',
					NGGALLERY_URLPATH . 'public/css/shutter-reloaded.css',
					false,
					'1.3.2',
					'screen'
				);
				wp_enqueue_style(
					'datepicker',
					plugins_url( 'css/jquery.ui.datepicker.css', __FILE__ ),
					false,
					'1.8.2',
					'screen'
				);
	...
	public function edit_current_screen( $screen ) {
		...
		switch ( $screen->id ) {
			case 'toplevel_page_' . NGGFOLDER:
				...
				$help .= '<p><strong>' . __(
					...
					'Shows some general information about your site, such as the number of pictures, albums and galleries.',
					'nggallery'
				) . '</p>';
				$help .= '<p><strong>' . __(
					'Latest News',
					'nggallery'
				) . '</strong> - ' . __( 'The latest NextCellent news.', 'nggallery' ) . '</p>';
				if ( ! is_multisite() || is_super_admin() ) {
					$help .= '<p><strong>' . __(
						'Related plugins',
						'nggallery'
					) . '</strong> - ' . __(
						'Shows plugins that extend NextCellent.',
						'nggallery'
					) . ' <strong>' . __(
						'Pay attention',
						'nggallery'
					) . '</strong>: ' . __(
						'third parties plugins that are compatible with NGG may not be 100% compatible with NextCellent Gallery!',
						'nggallery'
					) . '</p>';
				}
				$help .= '<p><strong>' . __(
					...
					'Shows general information about he plugin and some links.',
					'nggallery'
				) . '</p>';
				if ( ! ( get_locale() == 'en_US' ) ) {
					$help .= '<p><strong>' . __(
						'Translation',
						'nggallery'
					) . '</strong> - ' . __( 'View information about the current translation.' ) . '</p>';
				}
				if ( ! is_multisite() || is_super_admin() ) {
					$help .= '<p><strong>' . __(
						'Server Settings',
			...
			case "{$i18n}_page_nggallery-add-gallery":
				...
				$help = '<p>' . __(
					'On this page you can add galleries and pictures to those galleries.',
					'nggallery'
				) . '</p>';
				if ( nggGallery::current_user_can( 'NextGEN Add new gallery' ) ) {
					$help .= '<p><strong>' . __(
						...
						'Add new galleries to NextCellent.',
						'nggallery'
					) . '</p>';
				}
				if ( empty( $gallerylist ) ) {
					$help .= '<p><strong>' . __(
						'You must add a gallery before adding images!',
						'nggallery'
					) . '</strong>';
				} else {
					$help .= '<p><strong>' . __(
				...
				if ( wpmu_enable_function( 'wpmuImportFolder' ) && nggGallery::current_user_can( 'NextGEN Import image folder' ) ) {
					$help .= '<p><strong>' . __(
						...
						'Import a folder from the server as a new gallery.',
						'nggallery'
					) . '</p>';
				}
			...
			case "{$i18n}_page_nggallery-manage":
				...
				if ( ! isset( $_GET['mode'] ) || $_GET['mode'] === 'gallery' ) {
					...
					add_filter(
						...
						array( 'NGG_Gallery_List_Table', 'get_columns_static' ),
						0
					);
					$args = array(
					...
					add_filter(
						'manage_' . $screen->id . '_columns',
						array( 'NGG_Image_List_Table', 'get_columns_static' ),
						0
					);
					$args = array(
			...
			case "{$i18n}_page_nggallery-manage-album":
				$help = '<p>' . __(
					...
					'First select an album from the dropdown and then drag the galleries you want to add or remove from the selected album.',
					'nggallery'
				) . '</p>';
			...
			case "{$i18n}_page_nggallery-tags":
				$help = '<p>' . __(
					...
					'Rename, delete and edit tags. Use the rename function to merge tags.',
					'nggallery'
				) . '</p>';
			...
			case "{$i18n}_page_nggallery-options":
				$help = '<p>' . __(
					'Edit all of NextCellent\'s options. The options are sorted in multiple categories.',
					'nggallery'
				) . '</p>';
				$help .= '<p><strong>' . __(
					'General',
					...
					'General NextCellent options. Contains options for permalinks and related images.',
					'nggallery'
				) . '</p>';
				$help .= '<p><strong>' . __(
					'Images',
					...
					'All image-related options. Also contains options for thumbnails.',
					'nggallery'
				) . '</p>';
				$help .= '<p><strong>' . __(
					'Gallery',
					...
					'Everything about galleries. From sorting options to the number of images, it\'s all in here.',
					'nggallery'
				) . '</p>';
				$help .= '<p><strong>' . __(
					'Effects',
					...
					'Make your gallery look beautiful.',
					'nggallery'
				) . '</p>';
				$help .= '<p><strong>' . __(
					'Watermark',
					...
					'Who doesn\'t want theft-proof images?',
					'nggallery'
				) . '</p>';
				$help .= '<p><strong>' . __(
					'Slideshow',
					'nggallery'
				) . '</strong> - ' . __( 'Edit options for the slideshow.', 'nggallery' ) . '</p>';
			...
			case "{$i18n}_page_nggallery-style":
				$help = '<p>' . __(
					'You can edit the css file to adjust how your gallery looks.',
					'nggallery'
				) . '</p>';
				$help .= '<p>' . __(
					'When you save an edited file, NextCellent automatically saves it as a copy in the folder ngg_styles. This protects your changes from upgrades.',
					'nggallery'
				) . '</p>';
			...
			case "{$i18n}_page_nggallery-roles":
				$help = '<p>' . __(
					'You can assign the lowest user role that has access to a certain feature. Needless to say, all greater user roles will also have access to that feature.',
					'nggallery'
				) . '</p>';
				$help .= '<p>' . __(
					'NextCellent also works with various plugins that extend the default roles capabilities.',
					'nggallery'
				) . '</p>';
		...
		$screen->set_help_sidebar(
			...
			'<p><a href="http://codex.wordpress.org/Plugins_Editor_Screen" target="_blank">' . __(
				'Support Forums',
				'nggallery'
			) . '</a></p>' .
			'<p><a href="https://bitbucket.org/wpgetready/nextcellent" target="_blank">' . __(
				'Source Code',
				'nggallery'
			) . '</a></p>'
		);
