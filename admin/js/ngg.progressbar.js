...
nggProgressBar = {
	...
	init: function( s ) {
		...
		s = this.settings = jQuery.extend( {}, this.settings, {}, s || {} );
		width = Math.round( ( 100 / s.maxStep ) * 100 ) /100;
		// add the initial progressbar
		if ( jQuery( "#" + s.id + "_dialog" ).length == 0) {
			s.header = (s.header.length > 0) ? s.header : '' ;
			jQuery("body").append('<div id="' + s.id + '_dialog"><div id="' + s.id + '" class="progressborder"><div class="' + s.id + '"><span>0%</span></div></div></div>');
			jQuery('html,body').scrollTop(0); // works only in IE, FF
			// we open the dialog
			jQuery( "#" + s.id + "_dialog" ).dialog({
				width: 640,
				resizable : true,
				modal: true,
				title: s.header
	...
	addMessage: function( message ) {
		...
		if ( div.find("#" + s.id + "_message").length == 0)
			...
			jQuery("#" + s.id + "_message").html( message );
	},
	...
	addNote: function( note, detail ) {
		...
		if (detail)
			jQuery("#" + s.id + "_note").append("<li>" + note + "<div class='show_details'><span>[more]</span><br />" + detail + "</div></li>");
		else
			jQuery("#" + s.id + "_note").append("<li>" + note + "</li>");
	...
	increase: function( step ) {
		...
		jQuery("#" + s.id + " div").width( value );
		jQuery("#" + s.id + " span").html( rvalue );
	},

	finished: function() {
		s = this.settings;
		jQuery("#" + s.id + " div").width( '100%' );
		jQuery("#" + s.id + " span").html( '100%' );
		// in the case we add a note , we should wait for a click
		if (s.wait) {
			jQuery("#" + s.id).delay(1000).hide("slow");
			div.click(function () {
				jQuery("#" + s.id + "_dialog").dialog("destroy");
				jQuery("#" + s.id + "_dialog").remove();
			...
			window.setTimeout(function() {
				jQuery("#" + s.id + "_dialog" ).delay(4000).dialog("destroy");
				jQuery("#" + s.id + "_dialog").remove();
				// In the casee it's the manage page, force a submit
				jQuery('.nggform').prepend("<input type=\"hidden\" name=\"ajax_callback\" value=\"1\">");
				jQuery('.nggform').delay(4000).submit();
			}, 1000);
		}
	}
};